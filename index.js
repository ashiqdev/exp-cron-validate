...
const validator = (expression, limit, options = {}) => {
  ...
  if (daysOfMonth === '*' && months === '*' && daysOfWeek === '*') {
    if (hours && minutes === '0') {
      ...
      splittedHour.splice(0, 2);
      const joinHour = splittedHour.join('');

      const expressionInMs = Number(joinHour) * 3600000;
      if (expressionInMs < limit) {
        console.error('Limit is bigger than expression');
      }
      ...
      const splittedMinutes = minutes.trim().split('');
      splittedMinutes.splice(0, 2);

      const joinMinutes = splittedMinutes.join('');

      const minutesInMs = Number(joinMinutes * 60000);
      ...
      splittedSeconds.splice(0, 2);
      const joinedSeconds = splittedSeconds.join('');
      console.log({ joinedSeconds });

      secondsInMs = Number(joinedSeconds);
...
// at every quarter minute
expression = '*/1 * * * *';

// at every 2 hour
expression = '*/30 * * * * *';

validator(expression, 30000, { useSeconds: true });
